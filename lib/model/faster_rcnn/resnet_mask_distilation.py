...
from model.faster_rcnn.faster_rcnn_mask_distilation import _fasterRCNN
import torch
...
def build_feature_connector(t_channel, s_channel):
  C = [nn.Conv2d(s_channel, t_channel, kernel_size=3, stride=1, padding=1, bias=False),
       nn.BatchNorm2d(t_channel),nn.ReLU()]
...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False,etudiant=False):
  ...
  def _init_modules(self):
    ...
    if self.etudiant==True:
      self.Connectors = build_feature_connector(1024, 1024)
