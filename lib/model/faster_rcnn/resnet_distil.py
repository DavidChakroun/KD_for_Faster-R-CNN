...
from model.faster_rcnn.faster_rcnn_distil import _fasterRCNN
...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False,etudiant=False, numLayerTeacher=None):
    ...
    self.numLayerTeacher=numLayerTeacher
  ...
  def _init_modules(self):
    ...
    s_channels = resnet.get_channel_num()

    if self.etudiant==True:
        if self.numLayerTeacher<50:
            t_channels=[64, 128, 256, 512]
            self.Connectors = nn.ModuleList([build_feature_connector(t, s) for t, s in zip(t_channels, s_channels)])

        elif self.numLayerTeacher>49:
        ...
        teacher_bns = resnet.get_bn_before_relu()
  ...
  def extract_feature(self, x, preReLU=False):
      ...
      x = self.RCNN_base[0](x)
      x = self.RCNN_base[1](x)
      x = self.RCNN_base[2](x)
      x = self.RCNN_base[3](x)

      feat1 = self.RCNN_base[4](x)
      feat2 = self.RCNN_base[5](feat1)
      feat3 = self.RCNN_base[6](feat2)
      feat4 = self.RCNN_top(feat3)
