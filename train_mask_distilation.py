...
from model.faster_rcnn.resnet_mask_distilation import resnet
...
if __name__ == '__main__':
  ...
  if args.student == 'res101':
    fasterRCNN_etudiant = resnet(imdb.classes, 101, pretrained=True, class_agnostic=args.class_agnostic, etudiant=True)
  elif args.student == 'res50':
    fasterRCNN_etudiant = resnet(imdb.classes, 50, pretrained=True, class_agnostic=args.class_agnostic, etudiant=True)
  elif args.student == 'res34':
    fasterRCNN_etudiant = resnet(imdb.classes, 34, pretrained=True, class_agnostic=args.class_agnostic, etudiant=True)
  elif args.student == 'res18':
    fasterRCNN_etudiant = resnet(imdb.classes, 18, pretrained=True, class_agnostic=args.class_agnostic, etudiant=True)
  ...
  load_prof='../distillation/models/pascal_voc/res101/faster/pascal_voc_faster_rcnn_42_7_5001.pth'
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch ):
      ...
      loss_distil = (((torch.pow(prof_caracteristique - etudiant_caracteristiques_transformees, 2) * mask_batch).sum() )/norms)*0.01
    ...
    save_name = os.path.join(output_dir, '{}_{}_faster_rcnn_{}_{}_{}.pth'.format(args.student,args.dataset,args.session, epoch, step))
    save_checkpoint({
